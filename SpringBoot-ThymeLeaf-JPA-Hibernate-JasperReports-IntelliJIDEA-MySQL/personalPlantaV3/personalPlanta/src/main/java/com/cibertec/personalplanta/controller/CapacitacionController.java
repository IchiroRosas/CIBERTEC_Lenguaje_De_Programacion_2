...
public class CapacitacionController {
    ...
    @ResponseBody
    public ResponseEntity<List<EmpleadoDTO>> capacitacionDetalle(@PathVariable("id") Integer id) {
        List<EmpleadoDTO> empleados = empleadoService.listarEmpleadoPorCapacitacionId(id);
        if (empleados.isEmpty()) {
            return ResponseEntity.noContent().build(); // Devuelve 204 si no hay empleados
        }
        return ResponseEntity.ok(empleados); // Devuelve 200 con la lista de empleados
    }
